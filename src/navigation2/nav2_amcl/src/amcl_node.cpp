...
// rclcpp::parameterValue는 ros2에서 노드의 파라미터를 정의하고 사용할 때 사용하는 클래스임. 
  add_parameter(
...
AmclNode::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  particle_cloud_sensor_pub_->on_activate();
...
//  AMCL 알고리즘에서 로봇의 초기 위치를 샘플링할 때 사용함. 
#endif
// -------------------------------------------------------------------------------------------------------------
bool
AmclNode::getOdomPose(
  ...
  // 로봇의 오도메트리 좌표계에서의 위치와 방향을 저장하기 위한 변수임. 
  // rclcpp::Time은 sensor_timestamp은 센서의 타임스탬프를 나타내는 변수임.
  // std::string & frame_id는 로봇의 위치를 얻기 위해 사용되는 좌표계 id임. 
{
...
AmclNode::uniformPoseGenerator(void * arg)
{
  ...
  // 파티클 필터에서 로봇의 초기 위치와 방향을 무작위로 설정하는 과정
#else
...
void
// laser 데이터 받고, 해당 데이터를 통해 파티클 업데이트 하는 과정 같음 ~845 line까지 
...
// -----------------------------------------------------여기까지
  ...
  ldata.scores = new double[ldata.range_count];
  ...
  for (int i = 0; i < ldata.range_count; i++) {
    ...
    ldata.ranges[i][1] = angle_min +
      (i * angle_increment);
    double s = 1.0;
    if (i < static_cast<int>(laser_scan->intensities.size()))
      s = laser_scan->intensities[i];
    
    if (std::isnan(s) || s < 0.0) s = 0.0;
    else if (s > 1.0)             s = 1.0;
    ldata.scores[i] = s;
  }

  lasers_[laser_index]->sensorUpdate(pf_, reinterpret_cast<nav2_amcl::LaserData *>(&ldata));
  //   // 여기서 ‘센서업데이트 직후’ 파티클 클라우드 발행 
  pf_sample_set_t * set = pf_->sets + pf_->current_set;
  RCLCPP_INFO(get_logger(), 
    "Publish ParticleCloud: sensor-updated (before resampling).");
  publishParticleCloudSensorUpdate(set);
...
void 
AmclNode::publishParticleCloudSensorUpdate(const pf_sample_set_t * set)
{
  ...
  // ← 새 publisher
  particle_cloud_sensor_pub_->publish(std::move(cloud_with_weights_msg));
}
...
AmclNode::initPubSub()
{
  ...
  particle_cloud_pub_ = create_publisher<nav2_msgs::msg::ParticleCloud>(
    ...
    // 센서 업데이트 후 토픽: /particle_cloud_sensor_update
  particle_cloud_sensor_pub_ = create_publisher<nav2_msgs::msg::ParticleCloud>(
    "particle_cloud_sensor_update",
    rclcpp::SensorDataQoS());
