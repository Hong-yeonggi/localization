...
protected:
 ...
 // amcl_hyp_t 구조체는 amcl 알고리즘에서 사용되는 가설임. 
 // pf_vector_t는 추정 pose의 평균
 // pf_matrix_t는 위치와 방향에 대한 불확실성을 공분산 행렬로 나타냄.
  ...
  rclcpp_lifecycle::LifecyclePublisher<nav2_msgs::msg::ParticleCloud>::SharedPtr
    particle_cloud_pub_;
  rclcpp_lifecycle::LifecyclePublisher<nav2_msgs::msg::ParticleCloud>::SharedPtr particle_cloud_sensor_pub_;


  // rclcpp::Publisher<nav2_msgs::msg::ParticleCloud>::SharedPtr 
  //   particle_cloud_sensor_pub_;
  ...
  void publishParticleCloudSensorUpdate(const pf_sample_set_t * set);
